...
public class PrehladavanieDoHlbky extends Algoritmus {
    ...
    private void prehladavanieDoHlbky(Vrchol paVrchol) {
        ...
        for (Hrana hrana : graf.getHrany()) {
            if (hrana.getVrchol1().getNazov().equals(paVrchol.getNazov()) && !hrana.getVrchol2().getFarba().equals(Color.red)  ) {
                hrana.setFarba(Color.red);
                krokyAlgoritmu.add(graf.clone());
                prehladavanieDoHlbky(hrana.getVrchol2());

            } else if (hrana.getVrchol2().getNazov().equals(paVrchol.getNazov()) && !hrana.getVrchol1().getFarba().equals(Color.red) ) {
                hrana.setFarba(Color.red);
                krokyAlgoritmu.add(graf.clone());
